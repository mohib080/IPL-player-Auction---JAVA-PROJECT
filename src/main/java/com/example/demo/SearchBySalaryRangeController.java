...
public class SearchBySalaryRangeController {
    ...
    @FXML
    public void searchBySalaryRange() {
        if (collection == null) {
            playerDetailsArea.setText("Error: Player collection not initialized.");
            return;
        }

        int min = Integer.parseInt(minSalaryField.getText().trim());
        int max = Integer.parseInt(maxSalaryField.getText().trim());
        List<player> players = collection.searchBySalaryRange(min,max);

        VBox contentBox = new VBox(10);
        contentBox.setPadding(new Insets(10));

        if (players == null || players.isEmpty()) {
            TextArea textArea = new TextArea("No players available between this salary range." );
